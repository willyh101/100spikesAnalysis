function [outVars] = plotResponseOfRedCellsByCell(All,outVars,opts)
%%
numExps = numel(All);
...
for ind = 1:numExps
    ...
    for i= 1:numStims
        ...
        popRespRedCells{c} = squeeze(respMat(i,v,cellsToUse & isRed) - baseMat(i,v,cellsToUse & isRed));
...
outVars.popRespRedCells     = popRespRedCells;
...
popRespRedCells(isControl) = [];


figure(231)
clf
hold on
arrayfun(@(x) plot(outVars.numCellsEachEns(x), popRespRedCells{x},'ob'), 1:numel(popRespRedCells));
% sp = scatter(outVars.numCellsEachEns(ensToPlot),popRespRedCells(ensToPlot),'filled');
% sp.MarkerFaceAlpha = 0.6;
% sp.MarkerEdgeColor = 'none';
% hold on
% sp2=scatter(outVars.numCellsEachEns(ensToPlot)+0.75,popRespRedCells(ensToPlot),'filled');

function [All, outVars] = getEnsembleOSI_12(All, outVars)
...
for ind = 1:numel(All)
    ...
    for h = 1:numel(All(ind).out.exp.holoTargets)
        ...
        meanOri(h) = nanmean(idx2ori(All(ind).out.anal.prefOri(ht), [nan 0:30:330]));
        
        % mean ori by circular mean
        pos = idx2ori(All(ind).out.anal.prefOri(ht), [nan 0:30:330]);
        pos(isnan(pos)) = [];
        ...
        curve(1, :) = [];
        ensPO(h) = idx2ori(ensPOidx, [nan 0:30:330]);
        ensOSI(h) = osi_12(curve);
    end

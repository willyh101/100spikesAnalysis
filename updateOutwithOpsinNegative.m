...
for ind=4:numExps; ; %30:numExps;
    ...
    %         im1020ToLoad = fList(filePointer).name;
...
%                     imToUse = (imToUse-mn)./(mx-mn);
    ...
    [gridX gridY] = ndgrid(1:size(imToUse2,1),1:size(imToUse,2));
    ...
    for i = 1:nDepthsTotal
        ...
        dCoM = out.exp.allCoM(out.exp.allDepth==i,:)-out.exp.offsets;
        thisPlanevals=[];
        for k=1:size(dCoM,1)
...
%         threshold = prctile(opsinImage{i}(:),75);
        ...
        dCoM = out.exp.stimCoM(out.exp.stimDepth==i,:)-out.exp.offsets;

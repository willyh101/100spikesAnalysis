function pbal = powerCalib(PSTHs,powers,plotSummary)
...
for k = 1:numCells
    ...
    dataToUse = nanmean(PSTHsToUse(:,c,6:end),3);
    ...
    for i = 1:numel(powerList)
        p = powerList(i);
        theseResp = dataToUse(powersToUse==p);
    ...
    if isempty(dataforFit)
         satVal(k)= nan;
    satValLB(k) =  nan;
    Rsquare(k) = nan;
    expoUsed(k) = nan;
    
    fToUse = nan(size(0.001:0.001:max(powerList)*1.5));
    allFits{k} = [];
    else
    expoToTry = [4 6 8];
...
pbal.satVal =satVal; 
pbal.satValLB = satValLB;
pbal.Rsquare = Rsquare;
pbal.expoUsed =expoUsed;
pbal.allFits =allFits; 
pbal.FatSat =FatSat;

pbal.PowerAtSat = PowerAtSat; 
pbal.cellsThatFit =cellsThatFit; 
